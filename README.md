...
def add_contact(pb):
    dip = []
    for i in range(len(pb[0]) if pb else 5):
        if i == 0: dip.append(str(input("Enter name: ")))
        if i == 1: dip.append(int(input("Enter number: ")))
        if i == 2: dip.append(str(input("Enter e-mail address: ")))
        if i == 3: dip.append(str(input("Enter date of birth(dd/mm/yy): ")))
        if i == 4: dip.append(str(input("Enter category: ")))
...
def search_existing(pb):
    ...
    if choice == 1:
        ...
        query = int(input("Please enter the number: "))
        for i in range(len(pb)):
            if query == pb[i][1]:
                check = i
                temp.append(pb[i])
    # ... (Note: Logic continues for choice 3, 4, 5 as per previous transcription)
...
while ch in (1, 2, 3, 4, 5):
    ...
    if ch == 1:
        ...
        pb.clear() # Ensuring the list is cleared
    elif ch == 4:
